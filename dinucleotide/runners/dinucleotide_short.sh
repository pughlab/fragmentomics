...
outdir=$basedir/output/short
shdir=$basedir/sh_scripts/short
...
echo -e "samtools view -h $INPUTDIR/${bam}.bam | awk 'substr(\$0,1,1)==\"@\" || (\$9<=150 && \$9>=10) || (\$9>=-150 && \$9<=-10)' | \
...
### Format bedpe to bed +50bp
echo -e "awk '\$1==\$4' $outdir/${bam}.bedpe | \
awk -F \"\t\" 'OFS=\"\t\" { print \$1, \$2, \$6 }' | \
awk '! index(\$1, \"_\")' > $outdir/${bam}.bed\n" >> $shdir/${bam}.sh
...
echo -e "Rscript $frags/dinucleotide_get_contexts_short.R \
--id $bam \
