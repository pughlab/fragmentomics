...
option_list <- list(
  ...
  make_option(c("--bam"), type = "character", help = "bam file path. Required"),
...
## Run script
Healthy_Reads <- GetHealthyReads(bam)
write.table(Healthy_Reads, file.path(outdir, paste0(id, "_healthy_reads.txt")), sep = "\t", row.names = FALSE)
