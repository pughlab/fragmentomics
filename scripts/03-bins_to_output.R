...
option_list <- list(
  ...
  make_option(c("--bindir"), type = "character", help = "Path to bin files"),
...
bindir <- opt$bindir
outdir <- opt$outdir
filename <- file.path(outdir, paste0(id, "_bin_100kb.txt"))
filename_5mb <- file.path(outdir, paste0(id, "_bin_5Mb.txt"))
...
bins.list <- readRDS(file.path(bindir, paste0(id, "_bin_100kb.rds")))
tib.list <- as_tibble(bins.list)
rm(bins.list)
write.table(tib.list, filename, sep = "\t")
...
### Set arm levels and prune data. Ex:
### - Recalculate corrected fragments, corrected ratios, and coverage
...
### Center fragment and coverage ratios

df.fr3$ratio.centered <- (df.fr3$ratio.corrected2 - mean(df.fr3$ratio.corrected2))/sd(df.fr3$ratio.corrected2)
df.fr3$ratio.centered01 <- df.fr3$ratio.centered*0.01
df.fr3$coverage.centered <- (df.fr3$coverage.corrected2 - mean(df.fr3$coverage.corrected2))/sd(df.fr3$coverage.corrected2)
df.fr3$coverage.centered01 <- df.fr3$coverage.centered*0.01
...
df.fr3 <- df.fr3 %>%
  dplyr::rename(
    ...
    ratio_centered_01 = ratio.centered01,
    coverage_centered = coverage.centered,
    coverage_centered_01 = coverage.centered01,
    frag_gc = frag.gc2,
    coverage = coverage2
  )
...
df.fr3 <- df.fr3 %>%
  relocate(sample_id, seqnames, arm, start, end, gc, frag_gc, short, long, nfrags, ratio,
           ...
           mode_size,mean_size, median_size, q25_size, q75_size, binsize, bin, ratio_centered_01,
           coverage_centered_01)

write.table(df.fr3, filename_5mb, sep = "\t")
